...
struct PointLight : ShaderStruct {
  PointLight(const glm::vec3& position,
             ...
             const float exponentialAttenuation,
  ...
  float ambientFactor;  // Ambient factor (how much this light contributes to
                        // the global lighting in any case)

  float constantAttenuation;  // Constant attenuation factor of light with
                              // rising distance
  float linearAttenuation;  // Lienar attenuation factor of light with rising
                            // distance
  float exponentialAttenuation;  // Constant attenuation factor of light with
                                 // rising distance
