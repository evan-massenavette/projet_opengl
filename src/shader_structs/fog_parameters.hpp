...
struct FogParameters : ShaderStruct {
  ...
  FogParameters();
  FogParameters(const glm::vec3 &color, float linearStart, float linearEnd,
                float density, int equation, const bool isEnabled = true);
  ...
  float linearStart; // This is where linear fog starts (valid for linear
                     // equation only)
  float linearEnd;   // This is where linear fog ends (valid for linear equation
                     // only)
