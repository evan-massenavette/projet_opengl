...
PointLight::PointLight(const glm::vec3 &position, const glm::vec3 &color,
                       ...
                       const float exponentialAttenuation, const bool isOn)
    : position(position), color(color), ambientFactor(ambientFactor),
      constantAttenuation(constantAttenuation),
      linearAttenuation(linearAttenuation),
      exponentialAttenuation(exponentialAttenuation), isOn(isOn ? 1 : 0) {}

void PointLight::setUniform(ShaderProgram &shaderProgram,
  ...
  shaderProgram[constructAttributeName(uniformName, "ambientFactor")] =
      ambientFactor;
  shaderProgram[constructAttributeName(uniformName, "constantAttenuation")] =
      constantAttenuation;
  shaderProgram[constructAttributeName(uniformName, "linearAttenuation")] =
      linearAttenuation;
  shaderProgram[constructAttributeName(uniformName, "exponentialAttenuation")] =
...
void *PointLight::getDataPointer() const { return (void *)&position; }

const PointLight &PointLight::none() {
  static PointLight nonePointLight(glm::vec3(0.0f, 0.0f, 0.0f),
