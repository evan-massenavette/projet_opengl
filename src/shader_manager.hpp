...
class ShaderManager {
 public:
   ...
   *
   * @return True, if the shader was loaded and stored successfully or false
   * otherwise.
   */
  bool tryLoadGeometryShader(const std::string& key,
 ...
 private:
  ...
  std::map<std::string, std::unique_ptr<Shader>>
      _vertexShaderCache;  // Vertex shader cache - stores vertex shaders within
                           // their keys in std::map
  std::map<std::string, std::unique_ptr<Shader>>
      _fragmentShaderCache;  // Fragment shader cache - stores fragment shaders
                             // within their keys in std::map
  std::map<std::string, std::unique_ptr<Shader>>
      _geometryShaderCache;  // Gemetry shader cache - stores geometry shaders
                             // within their keys in std::map
};
