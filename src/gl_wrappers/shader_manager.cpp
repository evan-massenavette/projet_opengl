...
void ShaderManager::loadVertexShader(const std::string& key,
...
void ShaderManager::loadFragmentShader(const std::string& key,
                                       const std::string& filePath) {
...
void ShaderManager::loadGeometryShader(const std::string& key,
                                       const std::string& filePath) {
...
bool ShaderManager::tryLoadGeometryShader(const std::string& key,
                                          const std::string& filePath) {
  try {
...
const Shader& ShaderManager::getVertexShader(const std::string& key) const {
  if (!containsVertexShader(key)) {
    auto msg =
        "Attempting to get non-existing vertex shader with key '" + key + "'!";
    throw std::runtime_error(msg.c_str());
  }
...
const Shader& ShaderManager::getFragmentShader(const std::string& key) const {
  if (!containsFragmentShader(key)) {
    auto msg = "Attempting to get non-existing fragment shader with key '" +
               key + "'!";
    throw std::runtime_error(msg.c_str());
  }
...
const Shader& ShaderManager::getGeometryShader(const std::string& key) const {
  if (!containsGeometryShader(key)) {
    auto msg = "Attempting to get non-existing geometry shader with key '" +
               key + "'!";
    throw std::runtime_error(msg.c_str());
  }
